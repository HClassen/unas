...
from ..oneshot.layers import SharedBDWRConv2dModule
from ..mobilenet import FIRST_CONV_CHANNELS, LAYER_SETTINGS
from ..utils import make_divisible


__all__ = ["bdwrconv2d_choice_block_ctor", "BDWRConv2dSearchSpace"]


def bdwrconv2d_choice_block_ctor(
    in_channels: int,
    out_channels: int,
    width_mult: float,
    round_nearest: int,
...
def _sample_model(rng: np.random.Generator, width_mult: float) -> Model:
    """
    Randomly samples a single instance of a `Model` afrom the `ConvOp.BDWRConv2d`
...
class BDWRConv2dSearchSpace(SearchSpace):
